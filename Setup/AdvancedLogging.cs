...
namespace HallOfGundead
{
	public static class AdvancedLogging
	{
		...
		public static SGroup Log(object msg, Color32? col = null, bool HaveModName = false, bool HaveModIcon = false, SModifier[] modifiers = null)
		{
			...
			if (File.Exists(FLoorModModule.metadata.Archive))
			{
				if (HaveModIcon)
				{
					SImage icon = new SImage(FLoorModModule.metadata.Icon);
					sGroup.Children.Add(icon);
				}
			}
			if (HaveModName)
			{
				LogLabel modname = new LogLabel(FLoorModModule.metadata.Name + ": ");
			...
			foreach (string item in split)
			{
				...
				if (item.StartsWith("@("))
				{
					...
					Texture2D tex = ItemAPI.ResourceExtractor.GetTextureFromResource(path);
		...
		public static SButton LogButton(object msg, Color32? col = null, bool HaveModName = false, bool HaveModIcon = false)
		{
			...
			if (HaveModIcon == false)
			{
				...
				{
					img = new SButton($"{FLoorModModule.metadata.Name}: {msg}");
					img.Colors[0] = color;
					ETGModConsole.Instance.GUI[0].Children.Add(img);
				}
			}
			else
			{
				if (HaveModName == false)
				{
					...
					img.Colors[0] = color;
					if (File.Exists(FLoorModModule.metadata.Archive))
						img.Icon = FLoorModModule.metadata.Icon;
					ETGModConsole.Instance.GUI[0].Children.Add(img);
				}
				else
				{
